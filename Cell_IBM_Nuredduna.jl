...
@everywhere include("../Cell_IBM_Library.jl")
...
function phase_transition_parallel(L, M, lambda_ext)
	...
	println(f, "#Φ\tm_S\tErr_m_S\txi_S\tErr_xi_S\tm_R\tErr_m_R\txi_R\tErr_xi_R")

	result_k = SharedArray{Float64}(6)

    param_arr = [1, lambda_ext, 1, 1, 80]

    #Reduce computational time in the high mortality limit
    if lambda_ext >= 3.5
    ...
    PHI =  param_arr[end] * R_0_MF(param_arr; ρ_S=1.0) / param_arr[3]
